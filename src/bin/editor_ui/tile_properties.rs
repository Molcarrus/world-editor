...
use world_editor::{tileset,ui::widget::*};
...
impl TileTransform {
    ...
    fn into_basic(self) -> TileTransform {
        match self {
            ...
            TileTransform::Full(t) => TileTransform::Basic { y_translation: t.translation.y, y_rotation: t.rotation.to_euler(EulerRot::XYZ).1, scale: t.scale.x }
        }
    }
}
...
impl Default for TileTransform {
    fn default() -> Self {
        Self::Basic { y_translation: 0.0, y_rotation: 0.0, scale: 1.0 }
    }
}

impl From<Transform> for TileTransform {
    fn from(value: Transform) -> Self {
        let (translation, rotation, scale) = (value.translation, value.rotation, value.scale);
        ...
        if translation.x == 0.0
            ...
            && scale.x == scale.z 
        {
            TileTransform::Full(value).into_basic()
        } else {
            TileTransform::Full(value)
        }
    }
}

impl From<TileTransform> for Transform {
    fn from(value: TileTransform) -> Self {
        match value {
            TileTransform::Basic { y_translation, y_rotation, scale } => Transform { translation: Vec3::new(0.0, y_translation, 0.0), rotation: Quat::from_euler(EulerRot::XYZ,0.0, y_rotation, 0.0), scale: Vec3::splat(scale) },
    ...
    fn draw(&mut self, world: &mut World, ui: &mut egui::Ui, id: egui::Id) {
        ...
        if selection.is_changed() {
            let Some(tile) = tileset.tiles.get(&tile_ref.tile) else {
                ui.label(format!("Error: unknown tile {} in tileset {} ({:?})", tile_ref.tile, tileset.name, tile_ref.tileset));
                return;
            };
        ...
        let changed = match &mut self.transform {
            ...
            TileTransform::Basic { y_translation, y_rotation, scale } => {
                egui::Grid::new(id.with("basic"))
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.label("scale");
                        changed |= ui
                            .add(egui::DragValue::new(scale).speed(0.5).fixed_decimals(2))
                            .changed();
                        ui.end_row();
        ...
        for tile_ref in &selection.tiles {
            ...
            let Some(mut tile) = tileset.tiles.get_mut(&tile_ref.tile) else {
                warn!("Error: unknown tile {} in tileset {} ({:?})", tile_ref.tile, tileset.name, tile_ref.tileset);
                continue;
            };
        ...
        events.write(EditorUiEvent::RedrawMapTiles);
